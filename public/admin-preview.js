(() => {
  "use strict";
  if (window.__DFFDF_ADMIN_PREVIEW_V4__) return;
  window.__DFFDF_ADMIN_PREVIEW_V4__ = true;
  ...
  function themeFor(name) {
    ...
    return site.themes?.[name] || fallback[name] || fallback.midnight || {
      background:"#07070b", surface:"#11121a", surface2:"#181925", text:"#fff", muted:"#9b9ba8", accent:"#8065ff", accent2:"#b19cff"
    };
  }
  ...
  function applyAdminTheme() {
    ...
    const vars = {"--bg":t.background,"--surface":t.surface,"--surface2":t.surface2,"--text":t.text,"--muted":t.muted,"--accent":t.accent,"--accent2":t.accent2};
  ...
  function schedulePreview(delay = 100) {
    ...
    sendTimer = setTimeout(sendPreview, delay);
  }
  ...
  function openPreview() {
    if (!preview) {
      ...
      preview.querySelector("[data-preview-refresh]").onclick = () => {
        const frame = preview.querySelector("iframe");
        frame.src = "/?preview=1&refresh=" + Date.now();
      };
    ...
    // The public page performs its normal API load first. Waiting avoids that response
    // overwriting the editor state immediately after the first preview message.
  ...
  function installPreviewButtons() {
    ...
    if (trigger && !trigger.dataset.dffdfPreviewBound) {
      ...
      trigger.onclick = event => { event.preventDefault(); openPreview(); };
    }
  ...
  function themeNameFromCard(card) {
    return card?.dataset.theme || card?.dataset.name || card?.querySelector(".theme-info strong")?.textContent?.trim() || "";
  }
  ...
  function hookThemes() {
    ...
    grid.addEventListener("click",event => {
      ...
      const name = themeNameFromCard(card);
      if (!name) return;
      site.defaultTheme = name;
      grid.querySelectorAll(".theme-card").forEach(c => c.classList.toggle("selected",themeNameFromCard(c) === name));
      applyAdminTheme();
      schedulePreview();
      toast?.(`${name} selected — click Save Changes to keep it`);
    });
  }
    ...
    try {
      ...
      if (Array.isArray(pages)) {
        for (const page of pages) {
          const response = await fetch(`/api/admin/pages/${encodeURIComponent(page.slug)}`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(clone(page))});
  ...
  function hookSave() {
    const button = $("saveButton");
    if (!button || button.dataset.dffdfSaveBound) return;
    button.dataset.dffdfSaveBound = "1";
    button.onclick = saveEverything;
  }

  function bindInputs() {
    if (!ensureShape()) return;
    const fields = {siteNameInput:"siteName",logoInput:"logo",faviconInput:"favicon",descriptionInput:"siteDescription",siteUrlInput:"siteUrl",defaultPageInput:"defaultPage",backgroundInput:"backgroundImage",cursorInput:"cursor"};
    Object.entries(fields).forEach(([id,key]) => {
      const el=$(id); if(!el || el.dataset.dffdfBound) return;
      el.dataset.dffdfBound="1"; el.value=site[key] ?? "";
      const update=()=>{site[key]=el.value;schedulePreview();}; el.addEventListener("input",update); el.addEventListener("change",update);
    });
    const checks={maintenanceInput:"maintenance",loadingInput:"loadingScreen",transitionsInput:"pageTransitions"};
    Object.entries(checks).forEach(([id,key])=>{const el=$(id);if(!el||el.dataset.dffdfBound)return;el.dataset.dffdfBound="1";el.checked=site[key]!==false;el.addEventListener("change",()=>{site[key]=el.checked;schedulePreview();});});
    document.querySelectorAll("[data-glass]").forEach(el=>{
      if(el.dataset.dffdfBound)return;
      const key=el.dataset.glass;
      const converters={blur:v=>v+"px",transparency:v=>v/100,borderOpacity:v=>v/100,shadowStrength:v=>v/100,cornerRadius:v=>v+"px",animationSpeed:v=>v/100+"s",glowStrength:v=>v/100};
  ...
  function boot() {
    ...
    document.addEventListener("input",e=>{if(e.target.matches("input,textarea,select"))schedulePreview();},{passive:true});
    $("sectionPage")?.addEventListener("change",schedulePreview);
    window.addEventListener("message",event=>{
      if(event.origin===location.origin && event.data?.type==="DFFDF_PREVIEW_READY") schedulePreview(1200);
    });
  }

  // Bind immediately so the old inline save handler cannot win the race.
  boot();
  setTimeout(()=>{hookSave();installPreviewButtons();hookThemes();applyAdminTheme();},500);
  setTimeout(()=>{hookSave();installPreviewButtons();hookThemes();applyAdminTheme();},1200);
})();
